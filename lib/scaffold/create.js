...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'okcore-lib': '^' + bitcore.version,
    'okcore-node': version
    //'okcore-lib': 'okcashpro/okcore-lib',
    //'okcore-node': 'okcashpro/okcore-node'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/okcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the okcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the okcore-node directory and configuration
