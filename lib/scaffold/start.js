...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and okcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed okcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './okcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in okcore-node services directory
